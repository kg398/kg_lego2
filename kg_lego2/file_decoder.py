            ...
            for x in range(0,32):
                if model[z][y][x]!=0:                           # find brick by comparing corners
                    ...
                    if x0 < 31 and y0 < 13 and flag == 0:
                        if model[z][y0+3][x0+1] == model[z][y0][x0]:    # vertical 2x4 brick, 'r' = 0 and default picking location in centre of brick
                            bricks.append({'x':x0,'y':y0,'z':z,'r':0,'p':1,'xe':0,'ye':0,'b':0})
                            continue
                    if x0 < 29 and y0 < 15 and flag == 0:
                        if model[z][y0+1][x0+3] == model[z][y0][x0]:    # horizontal 2x4 brick, 'r' = 90 and default picking location in centre of brick
...
# add bricks to a model
def add_bricks(bricks,model):
    for i in range(0,len(bricks)):
        index = 0
        for y in range(0,16):
            for x in range(0,32):
                if model[bricks[i]['z']][y][x]>=index:
                    index = model[bricks[i]['z']][y][x]+1
        model[bricks[i]['z']][bricks[i]['y']][bricks[i]['x']] = index
        model[bricks[i]['z']][bricks[i]['y']+1][bricks[i]['x']] = index
        model[bricks[i]['z']][bricks[i]['y']][bricks[i]['x']+1] = index
        model[bricks[i]['z']][bricks[i]['y']+1][bricks[i]['x']+1] = index
        if bricks[i]['b']==0 and (bricks[i]['r'] == 0 or bricks[i]['r'] == 180):
            model[bricks[i]['z']][bricks[i]['y']+2][bricks[i]['x']] = index
            model[bricks[i]['z']][bricks[i]['y']+3][bricks[i]['x']] = index
            model[bricks[i]['z']][bricks[i]['y']+2][bricks[i]['x']+1] = index
            model[bricks[i]['z']][bricks[i]['y']+3][bricks[i]['x']+1] = index
        elif bricks[i]['b']==0 and (bricks[i]['r'] == 90 or bricks[i]['r'] == 270):
            model[bricks[i]['z']][bricks[i]['y']][bricks[i]['x']+2] = index
            model[bricks[i]['z']][bricks[i]['y']+1][bricks[i]['x']+2] = index
            model[bricks[i]['z']][bricks[i]['y']][bricks[i]['x']+3] = index
            model[bricks[i]['z']][bricks[i]['y']+1][bricks[i]['x']+3] = index
    return model
